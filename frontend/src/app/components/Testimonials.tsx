    ...
    entries: [
      {
        ...
        init: "S",
        color: "#9C27B0",
        highlight: "Mon ordi remis à neuf en moins de 24h.",
        text: "Il était infesté de virus et quasi inutilisable. Après intervention, il tourne mieux que le premier jour. Service rapide, professionnel et prix honnête. Je ne vais nulle part ailleurs.",
      },
      {
        kind: "testimonial",
        name: "Jean-Pierre",
        ...
        init: "J",
        color: "#FF9800",
        highlight: "Réseau d'entreprise configuré en une seule visite.",
        text: "Câblage, WiFi sécurisé, pare-feu et accès à distance — tout réglé en une journée. Explications claires, aucun jargon inutile. Maintenant sous contrat mensuel avec eux.",
      },
      {
        kind: "testimonial",
        name: "Karine",
        ...
        init: "K",
        color: "#009688",
        highlight: "Toutes mes données récupérées après crash total.",
        text: "Je pensais avoir tout perdu suite à une panne de disque dur — 8 ans de fichiers. Ils ont tout récupéré. Technicien extrêmement compétent et rassurant du début à la fin. Merci infiniment.",
      },
    ],
  },
  {
    ...
    entries: [
      {
        ...
        init: "M",
        color: "#E91E63",
        highlight: "Site livré en 3 semaines, premier page Google en 2 mois.",
        text: "Design magnifique, rapide à charger et mes clientes le trouvent facilement. L'équipe a su capter exactement l'identité de mon salon. Un investissement qui se rembourse chaque mois.",
      },
      {
        kind: "testimonial",
        name: "Benoit",
        ...
        init: "B",
        color: "#3F51B5",
        highlight: "Application sur mesure livrée dans les délais, sans surprise.",
        text: "Mes appels de service se gèrent maintenant via l'app. Communication parfaite tout au long du projet, aucun délai, aucune mauvaise surprise. Le résultat dépasse largement ce qu'on espérait.",
      },
      {
        kind: "testimonial",
        name: "Isabelle",
        ...
        init: "I",
        color: "#F44336",
        highlight: "+40 % de ventes en ligne dès le premier mois.",
        text: "Boutique e-commerce avec Stripe intégré, gestion de stock et livraison automatisée. Tout fonctionne parfaitement. Le soutien post-lancement est inclus et vraiment disponible.",
      },
    ],
  },
];
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <FadeUp delay={0.05}>
          <div style={{ display: "flex", justifyContent: "center", gap: "0.5rem", marginBottom: "3rem", flexWrap: "wrap" }}>
            {CATEGORIES.map((cat) => (
              <button key={cat.id} onClick={() => { setActiveTab(cat.id); setHovCard(null); }}
                style={{
                  fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: "0.88rem",
                  letterSpacing: "0.08em", textTransform: "uppercase", cursor: "pointer",
                  padding: "0.5rem 1.4rem",
                  background: activeTab === cat.id ? GREEN : "transparent",
                  color: activeTab === cat.id ? "#0c0c12" : GRAY,
                  border: `1px solid ${activeTab === cat.id ? GREEN : "rgba(255,255,255,0.1)"}`,
            ...
            <FadeUp key={entry.name} delay={i * 0.08}>
              <div onMouseEnter={() => setHovCard(i)} onMouseLeave={() => setHovCard(null)} style={{ height: "100%" }}>
                {entry.kind === "google"
                  ...
                  : <TestimonialCard entry={entry} hov={hovCard === i} />
                }
              </div>
            </FadeUp>
          ))}
